...
\title{\getGameName\ : Explore the Game Design of Unequal Communication Mechanic}
...
\begin{abstract}
  Inequality of communication capabilities, such as proficiency in a certain language, may cause an awkward experience in real life. However, in terms of gaming, this can make games more challenging and interesting. \textit{Charades}\cite{Charades} is a good example, where one player tries to express a word via body language, and the other tries to guess it. Besides, cooperative digital games with equal communication capabilities can be widely seen but few has adopted unequal setting. Therefore, we have designed Human and Dog, in which 2 players, a human and a dog, communicate to solve a series of puzzles with unequal communication capabilities. Game design was explored through the information type. Ultimately, we conducted an experiment to evaluate our game. From the collected data, several interesting communication patterns were discovered and we addressed some game design issues for future reference.
\end{abstract}
